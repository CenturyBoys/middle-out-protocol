...
class TestClass:
    ...
    def test_should_create_valid_method(self, method_type: MethodType, function: Function):
    ...
    def test_should_except_a_route_function_is_not_callable(self, method_type: MethodType):
        with pytest.raises(RouteFunctionIsNotCallableException) as error:
            _ = Method.create(type=method_type, function=Function.create(callback="not_callable"))
    ...
    def test_should_except_a_invalid_route_function_signature(
        ...
        with pytest.raises(RouteFunctionHasInvalidSignatureException) as error:
            _ = Method.create(type=method_type, function=Function.create(callback=lambda x: x))
